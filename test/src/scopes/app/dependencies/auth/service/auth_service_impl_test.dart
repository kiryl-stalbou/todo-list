...
import 'package:todo_app_krainet/src/scopes/app/dependencies/auth/service/auth_service_impl.dart';
...
void main() {
  group('AuthServiceImpl', () {
    ...
    test(
      ...
      expect(() async => authServiceImpl.signIn(testUser.email, testPassword), throwsA(const AuthException()));
      ...
      expect(() async => authServiceImpl.signIn(testUser.email, testPassword), returnsNormally);
    ...
    test(
      ...
      expect(() async => authServiceImpl.signUp(testUser.name, testUser.email, testPassword), returnsNormally);

      expect(authServiceImpl.authStateChanges, emits(testUser));

      expect(() async => authServiceImpl.signUp(testUser.name, testUser.email, testPassword), throwsA(const AuthException()));
    });
